...
public class OS_Service {
    ...
    public User Adjudication_User(Long userId){
        ...
        if(user_err!=null) {
            ...
            // 再次标记这个对象
            user_err.setLanguage(user_err.getLanguage() + "_User");
            // 需要将这个错误结果反馈到control模块 通过restTemplate方式
            log.info("错误反馈中.....");
            //  这个问题就是
            String ans = restTemplate.getForObject("http://localhost:9200/down_User/" + user_err.getLanguage()+"/User", String.class);
